...
Your AI twin runs on one brain (`claude -p`, no API key) with four plug-in modules wired in: **Slack**, **Gmail (email)**, a **Scheduler (cronjobs)**, and **long-term Memory**. Below are the capabilities those modules unlock — on their own and in combination. Everything happens in *your voice* (from your `PERSONALITY.md`).
